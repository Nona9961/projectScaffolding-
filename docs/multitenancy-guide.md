...
身份三元组随请求流转、异步经快照传播到 worker；提权/读放行状态仅作用于当前代码块，出块即恢复，不随线程传播、也不改变租户身份。
...
`server/src/test/java/com/nona/inf/persistence/tenant/TenantRepositoryAspectIntegrationTest.java`
...
`TenantRepositoryAspectIntegrationTest`（`crossTenantWrite*` 系列 + `elevatedWriteWithNullTenantShouldFail` /
...
| H3 | filter 对 bulk DML 生效 | 无参形态 `deleteAllInBatch()` / `deleteAll()` 受 filter 保护，异租户行删不掉（实验 D 转正：`TenantDmlBoundaryContractIntegrationTest#contractD2_noArgDeleteAllInBatchShouldBeFilteredToCurrentTenant`） | 依赖 Hibernate 版本行为，升级回归必查 |
...
- **不写身份上下文**：`CrossTenantAspect`（`HIGHEST_PRECEDENCE`，先于事务拦截器）以
...
作用域退出 auto-flush）会把跨租户篡改落库（实证：`TenantDmlBoundaryContractIntegrationTest#contractF_annotatedReadThenMutateBusinessFieldThenFlush`）。
...
读全空、写全拒（fail-fast）；调用追踪 API 则抛 `IllegalStateException`（fail-closed）。
